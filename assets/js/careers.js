  ...
  window.careerData.forEach((department, index) => {
    const departmentHtml = `
            <div class="department">
                <div class="department-header" onclick="toggleDepartment('${
                  department.id
                }')">
                    <h2>${department.title}</h2>
                    <span class="toggle-icon">${index === 0 ? "−" : "+"}</span>
                </div>
                <div class="job-listings ${index === 0 ? "active" : ""}" id="${
      department.id
    }">
                    ${department.jobs
                      .map(
                        ...
                        <div class="job-card">
                            ...
                            <button class="apply-btn" onclick="openApplyModal('${
                              job.title
                            }', '${department.title}')">Apply Now</button>
                        </div>
                    `
                      )
                      .join("")}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (careersList && window.careerData) {
    ...
    const firstDepartment = careersList.querySelector(
      ".department:first-child .job-listings"
    );
    if (firstDepartment) {
      firstDepartment.classList.add("active");
      firstDepartment.previousElementSibling.querySelector(
        ".toggle-icon"
      ).textContent = "−";
    }
  }
  ...
  if (applicationForm) {
    const emailInput = applicationForm.querySelector(
      'input[name="from_email"]'
    );
    const phoneInput = applicationForm.querySelector(
    ...
    phoneInput.addEventListener("input", function () {
      if (this.value && !isValidPhone(this.value)) {
        this.setCustomValidity(
          "Please enter a valid phone number (e.g., +977-9812345678)"
        );
        this.classList.add("invalid");
      } else {
        this.setCustomValidity("");
    ...
    applicationForm.addEventListener("submit", async function (event) {
      ...
      try {
        ...
        submitButton.textContent = "Submit Application";
        alert(
          "Failed to send the application. Please try again or contact us directly."
        );
      }
    });
  }
